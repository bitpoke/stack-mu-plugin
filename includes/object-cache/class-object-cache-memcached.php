 ...
 *
 * @package    Stack
 * @subpackage Object_Cache
 * @author     Calin Don <calin@presslabs.com>
 * @link       https://github.com/presslabs/stack-mu-plugin
 * @copyright  2019 Pressinfra
 */

namespace Stack\Object_Cache;

/**
 * Stack Memcached backend for WordPress object cache
 */
class Object_Cache_Memcached implements \Stack\ObjectCache {
	...
	/**
	 * Keeps track of updated options in this request so we can workaround
	...
	/**
	 * Keeps track of issued memcached commands
	 *
	 * @var array
	 */
	private $stats = array();
	...
	/**
	 * The request hash, used to store the keys to be prefetched
	 *
	 * @var string
	 */
	...
	public function __construct( $persistent_id = null ) {
		...
		add_action( 'add_option', __NAMESPACE__ . '\\' . __CLASS__ . '::mark_updated_option' );
		add_action( 'delete_option', __NAMESPACE__ . '\\' . __CLASS__ . '::mark_updated_option' );
		add_action( 'update_option', __NAMESPACE__ . '\\' . __CLASS__ . '::mark_updated_option' );
		...
		if ( ! defined( 'WP_CACHE_KEY_SALT' ) ) {
			define( 'WP_CACHE_KEY_SALT', '1' ); // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedConstantFound
		}

		$this->preload_enabled = ! defined( 'OBJECT_CACHE_PRELOAD' ) || ! OBJECT_CACHE_PRELOAD;
	...
	/**
	 * Closes the current object cache instance
	 */
	public function close() {
		$this->update_preload_keys();
		$this->cache = array();
	}


	/**
	 * Marks an option as updated for the current request life cycle.
	 *
	 * @param string $option the option name.
	 */
	public static function mark_updated_option( $option ) {
		if ( ! is_array( self::$updated_options ) ) {
			self::$updated_options = array();
		}
		if ( ! in_array( $option, self::$updated_options, true ) ) {
			self::$updated_options[] = $option;
		}
	}

	/**
	 * Checks if we should enable cache preloading behavior.
	 */
	private function should_preload() {
		if ( ! $this->preload_enabled ) {
			return false;
		}
		...
		return in_array( $_SERVER['REQUEST_METHOD'], array( 'GET', 'HEAD' ), true );
	}

	/**
	 * Pre-loads the object cache
	 */
	private function preload_cache() {
		...
		$this->request_hash = md5(
			wp_json_encode(
				array(
					$_SERVER['HTTP_HOST'], // phpcs:ignore WordPress.Security.ValidatedSanitizedInput
	...
	/**
	 * Saves the keys for preloading
	 */
	public function update_preload_keys() {
		if ( ! $this->should_preload() ) {
			return;
		}
	 ...
	 */
	public function get( $key, $group = 'default', $force = false, &$found = null, $server_key = '', $by_key = false, $cache_cb = null, &$cas_token = null ) {  // phpcs:ignore Generic.Files.LineLength.TooLong
		$derived_key = $this->buildKey( $key, $group );
		...
		if ( Memcached::RES_SUCCESS === $this->getResultCode() ) {
			if ( 'object-cache-preload' !== $group && isset( $value ) && ! in_array( $group, $this->no_mc_groups, true ) ) {
					$this->add_to_internal_cache( $derived_key, $value );
	 ...
	 * @return  bool|mixed                  Cached object value.
	 */
	public function getByKey( $server_key, $key, $group = 'default', $force = false, &$found = null, $cache_cb = null, &$cas_token = null ) {// phpcs:ignore Generic.Files.LineLength.TooLong
		/**
	...
	public function set( $key, $value, $group = 'default', $expiration = 0, $server_key = '', $by_key = false ) {
		...
		if ( 'alloptions' === $key && 'options' === $group ) {
			$found      = null;
			$alloptions = $this->get( $key, $group, true, $found, $server_key, $by_key, null, $cas_token );
			if ( $alloptions ) {
				foreach ( self::$updated_options as $option ) {
		...
		if ( Memcached::RES_SUCCESS === $result_code ) {
			...
			$this->delete( $key, $group, 0, $server_key, $by_key );
			return false;
		}
	...
	/**
	 * Returns the number of calls for each memcached method
	 *
	 * @return array The number of calls for each memcached method
	 */
